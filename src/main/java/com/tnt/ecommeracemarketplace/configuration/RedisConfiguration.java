...
//        redisTemplate.setKeySerializer(new StringRedisSerializer());
    ...
    public CacheManager productCacheManager(RedisConnectionFactory cf) {
        RedisCacheConfiguration redisCacheConfiguration = RedisCacheConfiguration.defaultCacheConfig()
            .serializeKeysWith(RedisSerializationContext.SerializationPair.fromSerializer(new StringRedisSerializer())) // 캐시의 키를 직렬화(serialize)하는 방식을 지정
            .serializeValuesWith(RedisSerializationContext.SerializationPair.fromSerializer(new GenericJackson2JsonRedisSerializer())) // 캐시의 값(value)을 직렬화하는 방식을 지정
